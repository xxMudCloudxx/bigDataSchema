...
| **数据管道工**      | **同学A**   | 1. **AI数据预处理（修复与扩充）** 2. 模拟实时数据流 3. 搭建 Flume -> Kafka 管道 | 1. `preprocess.py` 脚本 2. `simulate.py` 脚本 3. **[已交付]** 稳定的 `traffic-raw` Topic |
...
(我已完成本阶段全部任务，数据管道已于 11-18 全面打通并转入后台 7x24h 运行。)
...
- **行动 (同学B)**：
  ...
  2. **(更新) 配置 MyCat (`schema.xml`)**：
     ...
     - **配置 MySQL 自动清理 (!!! 新增重要任务 !!!)**：

       - **背景**：由于上游是 7x24 小时模拟数据，如果不清理，`db-app` 服务器磁盘将在 2 周内爆满。

       - **策略**：**“3天滚动清洗”**。我们只需要保留最近 3 天的数据供大屏展示。

       - **行动**：在 MySQL 中开启 Event Scheduler，并创建一个每日运行的 Event：
